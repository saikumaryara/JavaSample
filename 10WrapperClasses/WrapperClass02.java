class WrapperClass02 {
	public static void main(String[] args) {
		// i:-)Converting PDT(primitive datatype to WCO(wc object)(i.e storing pv in
		// WCO)
...
//   Integer io1=new Integer(5);//basically 5 is int type we are converting it into Integer
		...
		System.out.println(io1);// here our target is representing value as an object we can pass Integer type
								// anywhere
		...
		/*
		 * Immutable object::-Once object is created we can't change it's content (i.e
		 * pooling is applied) By default all WrapperClasses are Immutable objects
		 * Ex;-String.Integer,Long... Mutable Object::-The objects are allowed to
		 * change(i.e pooling is not applied) Ex:StrinBuilder,java.util.Date... pooling
		 * is appiled only byte range(i.e value -128 to 127) only and boolean also
		 * i.e(here if you are assign two same values it will create only 1 object it
		 * will create 1 object-->return true otherwise false
		 */
		...
		/*
		 * From JAVA 5.0V AutoBoxing(AB)::- AB Nothing But Converting PV to WCO directly
		 * without using Constructor(Integer.valueOf()) But The RULE is cnvrtg pv to wco
		 * ,wco is created based on value type, but not based on the assigned variable
		 * range type. compiler internally creates object using valueOf()
		 */
		Byte b1 = 5;// Byte b1=Byte.valueOf((byte)5);-->simly write like this Byte b1=5; from
					// java5.0v
		Byte b2 = 5;
		...
		System.out.println();
		/*
		 * ii-)Coverting WCO to PV (AutoUnboxing) (Retrieving pv from WCO)
		 */
		// Before JAVA5.0v
		int ia = io1.intValue();
		...
		/*
		 * AutoBoxing-->REsult is WCO AutoUnboxing --->Result is PDV
		 */

		/*
		 * We can't assign one pv to another pdt mapping WC ref var_ (AB),it leads to CE
		 * where as we can assign one WCO to another PDT var_ but rule is
		 */
		// Integer 1o6='a';
		int i8 = 'a';
		Character co2 = 'a';
